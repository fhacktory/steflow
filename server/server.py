...
def detect_hand():
    while(1):
        ...
        # Get fingertip points from contour hullGet fingertip points from contour hull
        # If points are in proximity of 80 pixels, consider as a single point in the group
        finger = []
        for i in range(0, len(hull) - 1):
            if (np.absolute(hull[i][0][0] - hull[i + 1][0][0]) > 120) or (np.absolute(hull[i][0][1] - hull[i + 1][0][1]) > 120):
                if hull[i][0][1] < 500:
...
socketio.run(app, host='0.0.0.0', port=5786)
